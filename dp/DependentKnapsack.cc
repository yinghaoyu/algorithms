...
class Code05_DependentKnapsack
{
 public:
  static const int MAXN = 33001;
  ...
  static int val[MAXM];
  static bool king[MAXM];  // king[i]表示 i 是不是主件

  static int fans[MAXM];  // fans[i] 表示 i 有多少件附件

  static int follows[MAXM][2];  // follows[i][0] follows[i][1]表示主件下哪两件附件
