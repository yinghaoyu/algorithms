...
class MakeASortedMinSwaps
{
 public:
  ...
  // 返回至少换几个数字
  static int minSwaps(vector<int> &A, vector<int> &B)
  {
    // 根据题意，B里的数字随意拿
    // 所以B里的数字排序，不会影响拿
  ...
  // pre 0、1
  // N*M*2
  static int process(vector<int> &A, vector<int> &B, int ai, int bi, int pre)
  {
    if (ai == A.size())
    {
      return 0;
    ...
    int p1 = preNum < A[ai] ? process(A, B, ai + 1, bi, 0) : INT32_MAX;
    // 可能性2，搞定当前的A[ai]，依靠交换
    int p2 = INT32_MAX;
  ...
  static int bs(vector<int> &B, int start, int num)
  {
    int ans = -1;
    int l = start;
    int r = B.size() - 1;
  ...
  static void test()
  {
    vector<int> A1 = {1, 8, 3, 6, 9};
    vector<int> B1 = {1, 3, 2, 4};
    cout << minSwaps(A1, B1) << endl;
    vector<int> A2 = {4, 8, 3, 10, 5};
    vector<int> B2 = {1, 3, 2, 4};
    cout << minSwaps(A2, B2) << endl;
    vector<int> A3 = {1, 8, 3, 6, 9};
    vector<int> B3 = {4, 3, 1};
    cout << minSwaps(A3, B3) << endl;
  }
};

int main()
{
  MakeASortedMinSwaps::test();
  return 0;
}
