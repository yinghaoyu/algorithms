...
class PavingTile
{
 public:
  ...
  {
    if (N < 1 || M < 1 || ((N * M) & 1) != 0)  // N*M是奇数也不满足条件
    {
      return 0;
    }
    // 只有一行，或者只有一列
  ...
  static int process(vector<int> &pre, int level, int N)
  {
    if (level == N)
    {
      // base case
  ...
  static int process2(int pre, int i, int N, int M)
  {
    if (i == N)
    {
      // base case
  ...
  static int dfs2(int op, int col, int level, int N, int M)
  {
    if (col == -1)
    {
  ...
  static int ways3(int N, int M)
  {
    ...
    int min = std::min(N, M);
    int pre = (1 << min) - 1;
    TdArray dp(1 << min, vector<int>(max + 1));
  ...
  static int process3(int pre, int i, int N, int M, TdArray &dp)
  {
    if (dp[pre][i] != -1)
    {
      return dp[pre][i];
  ...
  static int dfs3(int op, int col, int level, int N, int M, TdArray &dp)
  {
    if (col == -1)
    {
  ...
  static void dfs4(int way, int op, int index, int end, vector<int> &cur)
  {
    if (index == end)
    {
      cur[op] += way;
