...
class SplitBuildingBlock
{
 public:
  ...
  // 返回值：arr[i....]最少能分几个堆？
  static int zuo(vector<int> &arr, int x, int i, int r)
  {
    if (i == arr.size() - 1)
    {
      // 结算堆的数量
  ...
  // x差值，后 - 前 <= x
  static int minSplit(vector<int> &arr, int k, int x)
  {
    std::sort(arr.begin(), arr.end());
    int n = arr.size();
    vector<int> needs(n);  // 需要弥合的差值
    int size = 0;
    int splits = 1;  // 堆的数量
    for (int i = 1; i < n; i++)
    {
    ...
    // 试图去利用魔法积木，弥合堆！
    std::sort(needs.begin(), needs.begin() + size);
    for (int i = 0; i < size; i++)
    {
      int need = (needs[i] - 1) / x;  // 弥合需要几块
      if (k >= need)
      {
        splits--;  // 两堆变弥合成一堆
        k -= need;
