...
class CoverMax
{
 public:
  ...
  static void printLines(TdArray &lines)
  {
  ...
  static int maxCover1(TdArray &lines)
  {
    int min = INT32_MAX;
    int max = INT32_MIN;
    for (int i = 0; i < lines.size(); i++)
  ...
  static int maxCover2(TdArray &m)
  {
  ...
  // for test
  static TdArray generateLines(int N, int L, int R, int *len)
  {
    *len = getRandom(1, N);
    TdArray ans(*len, vector<int>(2));
    for (int i = 0; i < *len; i++)
    {
...
int main()
{
  ...
  {
    TdArray lines = CoverMax::generateLines(N, L, R, &len);
    // CoverMax::printLines(lines);
